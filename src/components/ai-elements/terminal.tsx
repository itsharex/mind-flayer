...
const resolveAnsiComponent = (moduleValue: unknown): ComponentType<AnsiComponentProps> => {
  ...
  if (typeof current !== "function") {
    throw new Error("Failed to resolve ansi-to-react component")
  }

  return current as ComponentType<AnsiComponentProps>
}

const Ansi = resolveAnsiComponent(AnsiModule)
...
export const TerminalHeader = ({ className, children, ...props }: TerminalHeaderProps) => (
  <div
    className={cn(
      "flex items-center justify-between border-zinc-800 border-b px-4 py-1",
      className
    )}
    {...props}
  >
...
export const TerminalCopyButton = ({
  ...
  return (
    <Button
      className={cn(
        "size-6 shrink-0 text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100",
        className
      )}
...
export const TerminalContent = ({ className, children, ...props }: TerminalContentProps) => {
  ...
  return (
    <div
      className={cn(
        "max-h-96 overflow-auto px-4 py-2 font-mono text-xs leading-relaxed",
        className
      )}
